...
src/app/            Next.js App Router
  ...
  register/         Onboarding wizard (3 steps, starts on Free)
...
src/lib/
  ...
  entitlements.ts   Resolves free / active / grace / expired → effective plan; can() / withinLimit()
...
## Billing flow
register → Free workspace → contextual paywall → `/billing` → `<PayPalCheckout>` (PayPal button or Visa/Mastercard card fields)
