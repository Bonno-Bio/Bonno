...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json(await runBackgroundJobs()); } catch (error) { console.error("[jobs] daily failed", error); captureException(error, { job: "daily" }); if (adminConfigured()) await adminDb().collection("job_runs").add({ type: "daily", status: "failed", startedAt: new Date().toISOString(), finishedAt: new Date().toISOString(), error: error instanceof Error ? error.message : "unknown" }); return NextResponse.json({ error: "Job runner failed" }, { status: 500 }); }
}
