...
function Invoices() {
  ...
  const { invoices, customers, business, recordPayment, addReminder, addInvoice, deleteInvoice, requestApproval, updateInvoice, convertQuoteToInvoice, notify } = useStore();
  ...
  return (
    <div>
      <PageHeader title="Invoices & Quotes" subtitle={`${ent.usage.invoices_per_month} / ${ent.limitFor("invoices_per_month") === Infinity ? "∞" : ent.limitFor("invoices_per_month")} invoices this month`}
        action={<div className="flex gap-2"><button className="btn-secondary" onClick={() => { const data = invoices.map((i) => ({ documentType: i.creditNoteForId ? "credit_note" : i.kind, number: i.number, customerId: i.customerId, issueDate: i.issueDate, dueDate: i.dueDate, status: i.status, lines: i.items, totals: invoiceTotals(i, vat), vatRate: vat, attachments: i.attachments ?? [] })); const blob = new Blob([JSON.stringify({ schema: "kgweboos.invoice.v1", exportedAt: new Date().toISOString(), business, records: data }, null, 2)], { type: "application/json" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `kgweboos-invoices-${todayISO()}.json`; a.click(); }}><Download size={15} /> Export</button><button className="btn-primary" onClick={() => setOpen(true)}><Plus size={16} /> New</button></div>} />
            ...
            return (
              <li key={i.id} className="card">
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  ...
                  {i.kind === "invoice" && i.status !== "draft" && i.status !== "void" && !i.creditNoteForId && <button className="btn-secondary px-2 py-1 text-xs" onClick={() => { const reason = prompt("Credit note reason"); if (!reason) return; addInvoice({ kind: "invoice", customerId: i.customerId, items: i.items.map((x) => ({ ...x, id: uid(), qty: -Math.abs(x.qty) })), issueDate: todayISO(), dueDate: todayISO(), status: "sent", notes: `Credit note for ${i.number}`, creditNoteForId: i.id, creditNoteReason: reason }); notify("Credit note issued", `Credit note created for ${i.number}.`); }}><FilePlus2 size={14} /> Credit note</button>}
    ...
    <Modal open={open} onClose={onClose} title={`New ${kind}`} wide>
      {limitHit ? <UpgradePrompt reason={upgradeReason("invoices_per_month")} /> : quoteLocked ? <UpgradePrompt reason={upgradeReason("quotes")} /> : (
        <div className="space-y-4">
          ...
          <Field label="Supporting document URL (optional)"><input className="input" value={attachmentUrl} onChange={(e) => setAttachmentUrl(e.target.value)} placeholder="Receipt, contract or delivery note link" /></Field>
      ...
      <div id="print-doc" className="rounded-xl border border-slate-200 p-5 text-sm">
        <div className="flex justify-between">
          ...
          <div className="text-right">
            <div className="text-xl font-bold uppercase text-emerald-700">{inv.creditNoteForId ? "Credit Note" : inv.kind === "invoice" ? (vat > 0 ? "Tax Invoice" : "Invoice") : "Quotation"}</div>
        ...
        <div className="ml-auto mt-3 w-56 space-y-0.5">
          ...
          {t.discount > 0 && <div className="flex justify-between text-rose-700"><span>Discount</span><span>-{money(t.discount)}</span></div>}
        ...
        {inv.creditNoteReason && <div className="mt-4 text-xs text-slate-500">Credit note reason: {inv.creditNoteReason}</div>}
        {inv.attachments?.map((a) => <a key={a.id} href={a.url} target="_blank" rel="noreferrer" className="mt-2 block text-xs text-emerald-700 underline">Supporting document: {a.name}</a>)}
