    ...
    (set, get) => {
      ...
      return {
        ...
        cancelPremium: () => {
          const sub = get().subscription;
          if (!sub) return;
          set({ subscription: { ...sub, plan: "free", status: "free", currentPeriodEnd: undefined, trialEndsAt: undefined } });
        ...
        simulateTrialEnd: () => {
          const sub = get().subscription;
          if (!sub) return;
          set({ subscription: { ...sub, plan: "free", status: "expired", trialEndsAt: new Date(Date.now() - 1000).toISOString(), currentPeriodEnd: undefined } });
          get().notify("Trial ended", "Your Premium trial has ended. Upgrade for P47/month to unlock everything again.");
        },
