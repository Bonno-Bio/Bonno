    ...
    (set, get) => {
      ...
      return {
        ...
        nextNumber: (kind) => {
          ...
          const nums = get().invoices.filter((i) => i.kind === kind && !i.creditNoteForId).map((i) => Number(i.number.replace(/\D/g, "")) || 0);
          return `${prefix}-${padNumber(Math.max(0, ...nums) + 1)}`;
        },
        addInvoice: (i) => {
          const rec: Invoice = { ...i, id: uid("inv"), businessId: bid(), number: i.creditNoteForId ? `CN-${padNumber(get().invoices.filter((x) => x.creditNoteForId).length + 1)}` : get().nextNumber(i.kind), createdAt: nowISO() };
