...
export async function runBackgroundJobs(now = new Date()): Promise<JobReport> {
  ...
  for (const business of businesses.docs) {
    ...
    await dispatchDueReminders(business.ref, now, report.reminders);
...
async function dispatchDueReminders(biz: DocumentReference, now: Date, result: JobResult) {
  const snap = await biz.collection("reminders").where("status", "in", ["scheduled", "failed"]).limit(50).get();
  for (const d of snap.docs) {
    const r = d.data(); const due = r.status === "scheduled" || !r.nextAttemptAt || new Date(r.nextAttemptAt).getTime() <= now.getTime(); if (!due || (r.attempts ?? 0) >= 3) continue;
    const customer = await biz.collection("customers").doc(r.customerId).get(); const c = customer.data() ?? {}; if (c.reminderConsent === false) { await d.ref.update({ status: "skipped_opt_out", updatedAt: FieldValue.serverTimestamp() }); continue; }
    const to = r.channel === "email" ? c.email : c.phone; if (!to) { await d.ref.update({ status: "failed", retryable: false, lastError: "No customer contact" }); continue; }
    await d.ref.update({ status: "sending", attempts: Number(r.attempts ?? 0) + 1, lastAttemptAt: now.toISOString() }); const resultMessage = await sendMessage({ to, body: r.message, channel: r.channel });
    await d.ref.update({ status: resultMessage.status, provider: resultMessage.provider, providerMessageId: resultMessage.providerMessageId ?? null, lastError: resultMessage.error ?? null, retryable: resultMessage.retryable, nextAttemptAt: resultMessage.retryable ? new Date(now.getTime() + 15 * 60_000).toISOString() : null });
    await biz.collection("audit_logs").add({ businessId: biz.id, actor: "system", action: `message.${resultMessage.status}`, entity: "reminder", entityId: d.id, createdAt: now.toISOString() }); result.changed++;
  }
}
