...
The daily job runner is available at `GET /api/jobs/daily`. Protect it with `JOBS_CRON_SECRET` and call it from Firebase Cloud Scheduler, Vercel Cron, or another trusted scheduler:

```bash
curl -H "Authorization: Bearer $JOBS_CRON_SECRET" https://your-domain.example/api/jobs/daily
```

It is idempotent and performs overdue marking, reminder scheduling, recurring invoice generation, monthly AI-credit reset, and Premium renewal notifications. It schedules reminders rather than pretending to deliver WhatsApp/SMS until a provider is connected.
